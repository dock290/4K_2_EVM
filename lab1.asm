...
;Работа с библиотекой ОС Kernel32.
;Внешние процедуры
EXTRN GetStdHandle: PROC
EXTRN WriteConsoleA: PROC
EXTRN ExitProcess: PROC ;Получить дескриптор окна.
;Вывести в консоль.
;Завершить процесс.
;Константы
...
Start:
    ...
    pushfd
    ;Получение регистра флагов через стек.
    pop EAX
    mov EBX, EAX
    xor EAX, 200000h ;Изменение 21-ого бита в регистре флагов.
    push EAX
    ;Сохранение и снова получение
    popfd
    ;регистра флагов.
    pushfd
    pop EAX
    cmp EAX, EBX
    ;Сравнение регистров до и после изменения.
    jne CPUIDSupp
    ;Если не изменился, то CPUID не поддерживается.
...
CPUIDSupp:
    ...
    cpuid
    mov Number, EAX
    ;Количество поддерживаемых функций.
    call Preobr
